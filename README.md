...
- [Mock Jutsu](https://github.com/altansayan/mock-jutsu-api) - Checksum-valid financial and identity test data generator (IBAN MOD-97, Luhn cards, TCKN, ISIN), plus real registered BIC/SWIFT codes - unlike format-only fakers, every value passes real validation.
...
- [greencheck](https://github.com/simin-yuan/greencheck) - Mutation testing aimed at the checker instead of the code: it mutates a known-good input and reports which mutations a validator, linter or CI quality gate fails to reject. Zero dependencies; on PyPI as `greencheck`.
...
- [mdoctest](https://github.com/ingrid-owusu/mdoctest) - Doctest for Markdown in any language: runs the console sessions and code blocks in your README/docs, verifies their output still matches, and rewrites them in place with `--fix`. Zero dependencies; ships as a pre-commit hook and a GitHub Action.
